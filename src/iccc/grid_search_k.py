...
def main():
    ...
    for k in K_VALUES:
        ...
        for cond_name, (yaml_name, gamma) in conditions.items():
            ...
            run_name = f"grid_k{k:.2f}_{cond_name}".replace(".", "p")
            metrics = model.val(
                data=yaml_path,
                conf=tau,
                iou=0.7,
                ...
                plots=False,
                name=run_name,
                project=os.path.join(ICCC_ROOT, "runs_grid"),
                exist_ok=True,
            )
    ...
    csv_path = os.path.join(ICCC_ROOT, "grid_search_k_results.csv")
    ...
    print("\n=== Average F1 across conditions per k ===")
    ...
    print(f"\nBest k = {best_k} (avg F1 = {summary[best_k]['avg_F1']:.4f})")

    summary_path = os.path.join(ICCC_ROOT, "grid_search_k_summary.json")
